...
def run_exp(parameter_yielder):
    for parameters in parameter_yielder:
        ...
        args = f"-d 3 -smt {smt} -rl {rl} -ent {ent} -n {epochs} -hs {hsize} -l {nlayer} -b {batch}".split(" ")
        print(threading.current_thread().name, "is running", " ".join(args))
        out_file = f"../../res/cb-{smt}-{rl}-{ent}_{batch}-{nlayer}-{hsize}.stdout"
        with open(out_file, "w") as f:
            subprocess.run(["python3",  "wafa.py", "c", "b", *args], stdout=f, stderr=subprocess.STDOUT)
