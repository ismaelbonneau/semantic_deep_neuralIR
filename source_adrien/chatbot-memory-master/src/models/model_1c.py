...
from model import KeyWordSelectionModel_1c
...
for i, (trainindices, testindices) in enumerate(all_but_one(indices, k=10)):
    ...
    decoder_archi = {"input_size": embedding_size + 4*hidden_size, "hidden_size": hidden_size, "num_layers": num_layers, "bidirectional":True, "dropout":0.2}
    predictor_structure = [2*hidden_size, 1]

    model = KeyWordSelectionModel_1c(encoder_archi, decoder_archi, predictor_structure)
    ...
    for epoch in range(500):
        ...
        if mean < best_model:
            best_model = mean
            shutil.copyfile(f"models_saves/fold{i}_1c_{epoch}_{mean}.torchsave", f"models_saves/fold{i}_1c_best.torchsave")
            delay = 0
        else:
            delay += 1
            if delay > max_delay:
