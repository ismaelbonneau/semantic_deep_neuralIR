...
for i, (trainindices, testindices) in enumerate(all_but_one(indices, k=10)):
    ...
    for epoch in range(500):
        ...
        torch.save({"model_dict": model.state_dict()}, f"models_saves/fold{i}_1b_{epoch}_{mean}.torchsave")

        if mean < best_model:
            best_model = mean
            shutil.copyfile(f"models_saves/fold{i}_1b_{epoch}_{mean}.torchsave", f"models_saves/fold{i}_1b_best.torchsave")
            delay = 0
        else:
            delay += 1
            if delay > max_delay:
