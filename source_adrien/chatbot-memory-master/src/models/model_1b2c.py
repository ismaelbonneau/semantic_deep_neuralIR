...
from model import KeyWordSelectionModel_1b, memory_2c
...
for i, (trainindices, testindices) in enumerate(all_but_one(indices, k=10)):
    ...
    model = memory_2c(KeyWordSelectionModel_1b)(encoder_archi, decoder_archi, predictor_structure)
    ...
    for epoch in range(500):
        ...
        torch.save({"model_dict": model.state_dict()}, f"models_saves/fold{i}_1b2c_{epoch}_{mean}.torchsave")

        if mean < best_model:
            best_model = mean
            shutil.copyfile(f"models_saves/fold{i}_1b2c_{epoch}_{mean}.torchsave", f"models_saves/fold{i}_1b2c_best.torchsave")
            delay = 0
        else:
            delay += 1
            if delay > max_delay:
